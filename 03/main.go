...
// Point represents a single point of the wire
type Point struct {
	x int
	y int
}

// Intersection represents an intersection along with it's details
...
// Wire defines a single wire on the path
type Wire struct {
	src    Point
	dest   Point
	length int
...
func processSingleRoute(route string, start Point) Point {
	direction := string(route[0])
	distance, err := strconv.ParseInt(route[1:len(route)], 10, 32)
	var destination Point

	if err != nil {
		panic(fmt.Sprintf("Error in parsing route %s:%s", route, route[1:len(route)]))
	}

	if direction == "R" {
		destination.x = start.x + int(distance)
		destination.y = start.y
	} else if direction == "D" {
		destination.x = start.x
		destination.y = start.y - int(distance)
	} else if direction == "L" {
		destination.x = start.x - int(distance)
		destination.y = start.y
	} else if direction == "U" {
...
func processInput(fullPath string) []Wire {
	s := strings.Split(fullPath, ",")
	wires := make([]Wire, 0, len(s))
	var curPoint = Point{x: 0, y: 0}

	for i := 0; i < len(s); i++ {
		dest := processSingleRoute(s[i], curPoint)

		var wire Wire
		wire.src.x = curPoint.x
		wire.src.y = curPoint.y
		wire.dest.x = dest.x
		wire.dest.y = dest.y
		wire.length = abs(dest.y-curPoint.y) + abs(dest.x-curPoint.x)
...
func getIntersectPoint(p1 Point, q1 Point, p2 Point, q2 Point) Point {
	var p Point
	...
	if determinant == 0 {
		...
		/* Do sanity checks */
		isXValid := ((x >= p1.x && x <= q1.x) || (x >= q1.x && x <= p1.x)) &&
			((x >= p2.x && x <= q2.x) || (x >= q2.x && x <= p2.x))
		isYValid := ((y >= p1.y && y <= q1.y) || (y >= q1.y && y <= p1.y)) &&
			((y >= p2.y && y <= q2.y) || (y >= q2.y && y <= p2.y))
...
func findIntersections(wiresA []Wire, wiresB []Wire) []Intersection {
	intersections := make([]Intersection, 0)
	stepsA := 0

	for i := 0; i < len(wiresA); i++ {
		stepsB := 0

		for j := 0; j < len(wiresB); j++ {
			tempPoint := getIntersectPoint(wiresA[i].src, wiresA[i].dest, wiresB[j].src, wiresB[j].dest)

			if tempPoint.x != 0 && tempPoint.y != 0 {
				delta := abs(tempPoint.x-wiresA[i].src.x) + abs(tempPoint.y-wiresA[i].src.y) + abs(tempPoint.x-wiresB[j].src.x) + abs(tempPoint.y-wiresB[j].src.y)
				intersection := Intersection{
					point:             tempPoint,
					manhattenDistance: abs(tempPoint.x) + abs(tempPoint.y),
					steps:             stepsA + stepsB + delta,
				}
				intersections = append(intersections, intersection)
				fmt.Printf("%+v\n", intersection)
			}
			stepsB += wiresB[j].length
		}
		stepsA += wiresA[i].length
	}

	return intersections
}

func findShortestIntersection(intersections []Intersection) int {
	var min = 1000000

	for i := 0; i < len(intersections); i++ {
		if min > intersections[i].steps {
			min = intersections[i].steps
		}
	}
...
func findClosestIntersection(intersections []Intersection) int {
	var min = 1000000

	for i := 0; i < len(intersections); i++ {
		if min > intersections[i].manhattenDistance {
			min = intersections[i].manhattenDistance
		}
	}
...
func main() {
	...
	// var pathA = "R8,U5,L5,D3"
	// var pathB = "U7,R6,D4,L4"

	wiresA := processInput(pathA)
	wiresB := processInput(pathB)

	fmt.Printf("%+v\n", wiresA)
	fmt.Printf("%+v\n", wiresB)

	intersections := findIntersections(wiresA, wiresB)
